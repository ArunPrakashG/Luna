...
namespace Assistant.Core.FileWatcher {
	...
	public class ModuleWatcher {
		...
		private void OnFileEventRaised(object sender, FileSystemEventArgs e) {
			...
			Task.Run(async () => await Core.ModuleLoader.ExecuteAsyncEvent(Modules.ModuleInitializer.MODULE_EXECUTION_CONTEXT.ModuleWatcherEvent, sender, e).ConfigureAwait(false));

			string fileName = e.Name;
			string absoluteFileName = Path.GetFileName(fileName);
			Logger.Log($"An event has been raised on module folder for file > {absoluteFileName}", LogLevels.Trace);
			...
			switch (absoluteFileName) {
				case "example.dll":
					Logger.Log("Ignoring example.dll file.", LogLevels.Trace);
					break;
