		...
		public void Main(string[] args) {
			using (ImapClient client = new ImapClient(new ProtocolLogger(Console.OpenStandardError()))) {
				...
				if (client.Inbox.Count > messages.Count) {
					...
					foreach (IMessageSummary message in client.Inbox.Fetch(messages.Count, -1, MessageSummaryItems.Full | MessageSummaryItems.UniqueId))
			...
			lock (idle.Client.SyncRoot) {
				...
				while (!idle.IsCancellationRequested) {
					...
					using (CancellationTokenSource timeout = new CancellationTokenSource()) {
						using (System.Timers.Timer timer = new System.Timers.Timer(9 * 60 * 1000)) {
