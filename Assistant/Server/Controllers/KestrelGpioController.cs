...
namespace HomeAssistant.Server.Controllers {
	...
	public class KestrelGpioController : Controller {
		...
		public ActionResult<GenericResponse<string>> GetAllPinStatus() {
			...
			try {
				List<GPIOPinConfig> config = AssistantCore.Core.Controller.GPIOConfig;
		...
		public ActionResult<GenericResponse<string>> GetPinStatus(int pinNumber) {
			...
			try {
				GPIOPinConfig config = AssistantCore.Core.Controller.FetchPinStatus(pinNumber);
		...
		public ActionResult<GenericResponse<string>> GetRelayPinStatus() {
			...
			foreach (int pin in AssistantCore.Core.Config.RelayPins) {
				resultConfig.Add(AssistantCore.Core.Controller.FetchPinStatus(pin));
			}
		...
		public ActionResult<GenericResponse<string>> GetIrSensorStatus() {
			...
			foreach (int pin in AssistantCore.Core.Config.IRSensorPins) {
				resultConfig.Add(AssistantCore.Core.Controller.FetchPinStatus(pin));
			}
	...
	public class KestrelGpioConfigController : Controller {
		...
		public ActionResult<GenericResponse<string>> SetPinStatus(int pinNumber, PinMode pinMode, bool isOn) {
			...
			bool result = AssistantCore.Core.Controller.SetGPIO(pinNumber, pinMode == PinMode.Output ? GpioPinDriveMode.Output : GpioPinDriveMode.Input,
		...
		public ActionResult<GenericResponse<string>> RelayCycle(GPIOCycles cycleMode) {
			...
			Helpers.InBackgroundThread(async () => await AssistantCore.Core.Controller.RelayTestService(cycleMode).ConfigureAwait(false), "Relay Cycle");
