...
namespace HomeAssistant.AssistantCore {
	...
	public class Core {
		...
		public static string AssistantName { get; private set; } = "TESS";
		...
		public static async Task<bool> InitCore(string[] args) {
			...
			if (Helpers.CheckForInternetConnection()) {
				...
				Logger.Log($"Starting {Core.AssistantName} in offline mode...");
			...
			try {
				...
				Logger.Log($"X---------------- Starting {AssistantName} Assistant v{Constants.Version} ----------------X", LogLevels.Ascii);
		...
		private static async Task PostInitTasks() {
			...
			TTSService.SpeakText($"{Core.AssistantName} Home assistant have been sucessfully started!", SpeechContext.AssistantStartup, true);
