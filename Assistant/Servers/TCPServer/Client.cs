...
using Assistant.Servers.TCPServer.Events;
using Assistant.Servers.TCPServer.Responses;
...
using static Assistant.Servers.TCPServer.CommandEnums;

namespace Assistant.Servers.TCPServer {
	public class Client {
		private readonly Logger Logger;
		private CommandObject? PreviousCommand { get; set; }
		...
		private async Task ProcessMessage(CommandObject cmdObject) {
			...
			if(cmdBase.Command == Command.Disconnect) {
				return;
			}
		...
		private string? OnClientCommandType(CommandBase command) {
			...
			Logger.Log("Client command recevied!", LogLevels.Trace);
			switch (command.Command) {
				...
				case Command.GetAssistantInfo:
					string r = new AssistantInfoResponse().AsJson();
					if (!string.IsNullOrEmpty(r)) {
						return FormatResponse(CommandResponseCode.OK, ResponseObjectType.AssistantInfo, "Assistant Info", r);
					}
					return null;
				case Command.GetPiInfo:
					string? response = new PiInfoResponse().GetPiInfo();

					if (!string.IsNullOrEmpty(response)) {
						return FormatResponse(CommandResponseCode.OK, ResponseObjectType.PiInfo, "Pi Info", response);
					}
					return null;
				case Command.ShutdownAssistant:
					Helpers.ScheduleTask(async () => await Core.Exit(0).ConfigureAwait(false), TimeSpan.FromSeconds(10));
					return FormatResponse(CommandResponseCode.OK, ResponseObjectType.NoResponse, "Shutting down in 10 seconds");
				case Command.ShutdownPi:
					Helpers.ScheduleTask(async () => await Core.SystemShutdown().ConfigureAwait(false), TimeSpan.FromSeconds(10));
					return FormatResponse(CommandResponseCode.OK, ResponseObjectType.NoResponse, "Shutting down in 10 seconds");
				case Command.RestartAssistant:
					Helpers.ScheduleTask(async () => await Core.Restart(0).ConfigureAwait(false), TimeSpan.FromSeconds(10));
					return FormatResponse(CommandResponseCode.OK, ResponseObjectType.NoResponse, "Restarting in 10 seconds");
				default:
					return null;
			}
		}
		...
		private string? OnGpioCommandType(CommandBase command) {
			...
			Logger.Log("Gpio command recevied!", LogLevels.Trace);
			switch (command.Command) {
		...
		private string? OnRemainderCommandType(CommandBase command) {
			...
			Logger.Log("Remainder command recevied!", LogLevels.Trace);
			switch (command.Command) {
